    ...
    <div id="app-container" className="w-full min-h-screen bg-[#070a13] text-slate-100 font-sans p-4 md:p-6 flex flex-col gap-5 overflow-x-hidden">
      ...
      <main id="app-bento-grid" className="flex-1 grid grid-cols-1 md:grid-cols-12 gap-5">
        ...
        <section id="ai-consult-and-techniques-section" className="col-span-1 md:col-span-4 flex flex-col gap-5">
          ...
          {/* AI Sentinel Chat (Bento Card) */}
          <div id="ai-advisor-card" className="bg-[#0b0f1a] border border-indigo-500/20 rounded-3xl flex flex-col h-[460px] shadow-2xl relative overflow-hidden">
            
            {/* Advisor Header */}
            <div className="p-4 border-b border-indigo-500/20 flex items-center justify-between bg-indigo-950/40">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-xs font-bold shadow-md shadow-indigo-900/40 animate-pulse">
                  <Bot className="h-4 w-4 text-white" />
                </div>
                <div>
                  <p className="font-bold text-sm text-slate-200">Sentinel Advisor</p>
                  <p className="text-[10px] text-indigo-400 flex items-center gap-1 font-mono">
                    <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-ping" />
                    Structural AI Engine Online
                  </p>
                </div>
              </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`p-3 rounded-2xl text-xs leading-relaxed ${
                      ...
                      {/* Very simple markdown formatter to avoid bulky imports */}
                      {m.content.split('\n').map((para, idx) => {
                        let formatted = para;
                        // Replace bold markup
            ...
            <div className="px-4 py-2 border-t border-slate-800/40 bg-[#090d16] overflow-x-auto whitespace-nowrap flex gap-1.5 scrollbar-none">
            ...
            {/* Chat input form */}
            <form onSubmit={handleSendMessage} className="p-3 bg-slate-950 flex gap-2 border-t border-slate-800">
              <input
                type="text"
                value={inputMessage}
